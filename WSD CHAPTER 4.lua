...
getgenv().BreakLoopAutoSeat = false
...
local function seat()
  if game.Players.LocalPlayer.Character.Humanoid.Sit == true then return end
  local old = rootpart.CFrame
  rootpart.CFrame = CFrame.new(151.418716, 192.515686, 82.5042191, 1, 0, 0, 0, 0.998586237, 0.0531555787, 0, -0.0531555787, 0.998586237)
  for i, v in pairs(workspace.Seats:GetDescendants()) do
    if v.Name == "Primary" and v.Parent:IsA("Model") then
      local jarak = (v.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
      if jarak <= 5 then
...
local function drink()
  ...
  if game.Players.LocalPlayer.Backpack:FindFirstChild("Drinking Glass") == nil or game.Players.LocalPlayer.Backpack:FindFirstChild("Water of Glass") == nil then
    ...
    task.wait(0.1)
    rootpart.CFrame = old
  elseif game.Players.LocalPlayer.Backpack:FindFirstChild("Drinking Glass") ~= nil or game.Players.LocalPlayer.Backpack:FindFirstChild("Water of Glass") ~= nil then
    for i, v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
      if v.Name == "Drinking Glass" then
        game.Players.LocalPlayer.Character.Humanoid:EquipTool(v)
        game.Players.LocalPlayer.Character.Humanoid:EquipTool(v)
...
Tabs.Main:AddButton({
    ...
    Callback = function()
      ...
      if game.Players.LocalPlayer.Backpack:FindFirstChild("Hammer") ~= nil then
...
Tabs.Main:AddButton({
    ...
    Description = "",
    Callback = function()
      if getgenv().Active == false and workspace.Game.PossesedMom.ChaseDoor.Value == false then
        repairwindow()
      end
    end
})
...
local Toggle = Tabs.Main:AddToggle("Auto Seat", 
{
    Title = "Auto Seat (BETA)", 
    Description = "",
    Default = false,
    Callback = function(state)
        if state == true then
          ...
          while wait() do
            task.wait(0.2)
            if getgenv().Active == false and workspace.Game.PossesedMom.ChaseDoor.Value == true and game.Players.LocalPlayer.Character.Humanoid.Sit == false then
              seat()
            end
            if getgenv().BreakLoopAutoSeat == true then
              break
            end
          end
        end
        if state == false then
            getgenv().BreakLoopAutoSeat = true
        end
    end 
})
