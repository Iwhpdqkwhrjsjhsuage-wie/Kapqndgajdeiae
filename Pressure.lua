    ...
    Callback = function(state)
      if state == true then
        autoloot = game:GetService("ProximityPromptService").PromptShown:Connect(function(prompt)
          if prompt:IsA("Part" ) or prompt:IsA("MeshPart" ) and prompt.Name == "HighLight" or prompt.Name == "Door" or prompt.Name == "ProxyPart" then            
            fireproximityprompt(prompt.ProximitPrompt)
          end
        end)
      end
      if state == false then
