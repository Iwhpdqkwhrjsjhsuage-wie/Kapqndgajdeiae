...
local function applykey2(inst)
  ...
  while wait() do
    ...
    if getgenv().BreakLoopKey == true or inst == nil then
      break
    end
  end
end
...
local function applymos(inst)
 ...
 text.MaxDistance = 300
  ...
  dec.Texture = "http://www.roblox.com/asset/?id=9180622665"
  dec.ZIndex = 10
...
{
    Title = "Auto Loot V1 (Recommended)", 
    Description = "",
    Default = false,
    Callback = function(state)
      if state == true then
    ...
    Callback = function(state)
      if state == true then
        autoloot = game:GetService("ProximityPromptService").PromptShown:Connect(function(prompt)
          local loot = coroutine.create(function()
            for _, v in ipairs(workspace.Rooms:GetDescendants()) do
...
local Toggle = Tabs.Main:AddToggle("Avoid Monster V1", 
{
    ...
    Callback = function(state)
      if state == true then
        ...
        p.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 2000, 0)
        p.Name = "NevcitPart"
        p.Transparency = 1
        p.Anchored = true  
        p.Parent = workspace
