...
local function applynormaldoor1(inst)
	...
	text.Name = "NevcitNormalDoor"
	text.Adornee = inst
	text.Size = UDim2.new(20, 0, 20, 0)
	text.StudsOffset = Vector3.new(0, 1, 0)
	text.AlwaysOnTop = true
	text.Parent = inst
 local f = Instance.new("BillboardGui")
	f.Name = "NevcitNormalDoor1"
 f.Adornee = inst
	f.Size = UDim2.new(1, 0, 1, 0)
	f.StudsOffset = Vector3.new(0, -0.5, 0)
	f.AlwaysOnTop = true
...
local Toggle = Tabs.Main:AddToggle("Auto Loot (NO LAG)", 
{
    Title = "Auto Loot ( No Lag )", 
    Description = "",
    Default = false,
    Callback = function(state)
      if state == true then
        autoloot = game:GetService("ProximityPromptService").PromptShown:Connect(function(prompt)
          for _, v in ipairs(workspace.Rooms:GetDescendants()) do
            if v:IsA("MeshPart") or v:IsA("Part") then
              if v.Name == "Main" or v.Name == "Door" or v.Name == "ProxyPart" then
                local distancer = (v.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
                if distancer <= 9 then
          ...
          for _, v in ipairs(workspace.Rooms:GetDescendants()) do
            if v:IsA("Model") then
              if v.Name == "BigFlashBeacon" or v.Name == "FlashBeacon" or v.Name == "Flashlight" or v.Name == "Medkit" or v.Name == "Lantern" or v.Name == "SmallLantern" or v.Name == "WindupLight" or v.Name == "CodeBreacher" or v.Name == "Blacklight" or v.Name == "DefaultBattery1" or v.Name == "DefaultBattery2" or v.Name == "DefaultBattery3" or v.Name == "AltBattery1" or v.Name == "AltBattery2" or v.Name == "AltBattery3" or v.Name == "RoomsBattery" or v.Name == "Gummylight" then
                local distancee = (v.ProxyPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
                if distancee <= 7 then
                  fireproximityprompt(v.ProxyPart.ProximityPrompt)
                end
              end
      ...
      if state == false then
        autoloot:Disconnect()
        autolootroom:Disconnect()
      end
    end 
})

local Toggle = Tabs.Main:AddToggle("Auto Loot", 
{
    Title = "Auto Loot (Laggy)", 
    Description = "",
    Default = false,
    Callback = function(state)
      if state == true then
...
local Toggle = Tabs.Main:AddToggle("Avoid Monster (Laggy)", 
{
    Title = "Avoid Monster (Laggy)", 
    Description = "",
    Default = false,
    Callback = function(state)
      if state == true then
          ...
          if v.Name == "Angler" or v.Name == "Pinkie" or v.Name == "Chainsmoker" or v.Name == "Blitz" or v.Name == "Froger" or v.Name == "Pandemonium" or v.Name == "RidgeAngler" or v.Name == "RidgePinkie" or v.Name == "RidgeChainsmoker" or v.Name == "RidgeBlitz" or v.Name == "RidgeFroger" then
            ...
            task.spawn(function()
              while wait() do
                ...
                if v == nil or getgenv().BreakLoopAvoid == true then
                  break
                end
              end
            end)
            v.Destroying:Wait()
            getgenv().BreakLoopAvoid = true
...
local Main = game:GetService("CoreGui").ScreenGui:GetChildren()[2]
...
TextButton.Size = UDim2.new(0.06, 0, 0.06, 0)
